...
void connect_wifi(char *ssid,char *password)
{
	...
	Show_Str(40,150,200,16,"start connect_wifi",16,0); 
	...
	while(atk_8266_send_cmd(p,"WIFI GOT IP",500))					//连接目标路由器,并且获得IP
	{
		...
		if(times>=0)
			break;
		sprintf((char*)p,"\"%d\"AT+CWJAP=\"%s\",\"%s\"",times,ssid,password);//设置无线参数:ssid,密码
		Show_Str_Mid(0,60+30*times,p,16,240); 
		sprintf((char*)p,"AT+CWJAP=\"%s\",\"%s\"",ssid,password);//设置无线参数:ssid,密码
	}
	Show_Str(40,200,200,16,"wifi is connect",16,0); 
...
void sta_connect_tcp()
{
	
	char *ssid = "zxh";
	char *password = "12345678";
	char *ip = "192.168.177.115";
	char *port = "8086";
