...
u8 atk_8266_wifista_test(void)
{
	...
	sprintf((char*)p,"AT+CWJAP=\"%s\",\"%s\"","zxh","12345678");//设置无线参数:ssid,密码
	while(atk_8266_send_cmd(p,"WIFI GOT IP",300))					//连接目标路由器,并且获得IP
	{
		
		times++;
		sprintf((char*)p,"\"%d\"AT+CWJAP=\"%s\",\"%s\"",times,wifista_ssid,wifista_password);//设置无线参数:ssid,密码
...
PRESTA:
	...
	else     //TCP
	{
		if(netpro&0X01)     //TCP Client    透传模式测试
		{
			...
			sprintf((char*)p,"AT+CIPSTART=\"TCP\",\"%s\",%s","192.168.177.115","8086");   
			while(atk_8266_send_cmd(p,"OK",500))
			{
					LCD_Clear(WHITE);
					POINT_COLOR=RED;
