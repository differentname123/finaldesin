...
public class total_key_word {
	...
	static boolean str_zhuanhuan(String recentname)//判断是否这个文件以经是最新文件
	{
		boolean flag = false;
		String str_re = null;
		...
		for(int i = 0; i < fileNameLists.length; i ++){
			...
			{
				try {
					str_re  = kword.get_keyword(fileNameLists[i],appid,api_key,path);
				} catch (UnsupportedEncodingException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
				System.out.println("转换结果" + str_re);
				write_recent_name(recent_name_path,""+x);//该进行记录的回写
				write_string(keydata_path,str_re,x+".txt");
				nowtime = x;
