...
const u8* wifista_ssid="zxh";			//路由器SSID号
const u8* wifista_encryption="wpawpa2_aes";	//wpa/wpa2 aes加密方式
const u8* wifista_password="12345678"; 	//连接密码


const u8* zxh_wifista_ssid="zxh";			//路由器SSID号
const u8* zxh_wifista_encryption="wpawpa2_aes";	//wpa/wpa2 aes加密方式
const u8* zxh_wifista_password="12345678"; 	//连接密码

//WIFI AP模式,模块对外的无线参数,可自行修改.
const u8* wifiap_ssid="Hsiaohuchu11";			//对外SSID号
const u8* wifiap_encryption="wpawpa2_aes";	//wpa/wpa2 aes加密方式
const u8* wifiap_password="zxh111111"; 		//连接密码 
...
//	u8 timex;
	...
	while(atk_8266_send_cmd("AT","OK",20))//检查WIFI模块是否在线
	{
		...
		zxh_atk_8266_wifista_test1();	//WIFI AP测试
}
...
void sendStr(char *str)
{
	...
	//Show_Str(30+34,200,156,12,"go into sendStr",12,0);
	
	atk_8266_quit_trans();
	//Show_Str(30+34,230,156,12,"over atk_8266_quit_trans",12,0);
	atk_8266_send_cmd("AT+CIPSEND","OK",20);         //开始透传  
	//Show_Str(30+34,260,156,12,"over atk_8266_send_cmd",12,0);	
	sprintf((char*)p,"%s\r\n",str);//测试数据
	//Show_Str(30+34,290,156,12,"over sprintf",12,0);
	Show_Str(30+54,100,300,12,p,12,0);
	u3_printf("%s",p);
	//Show_Str(30+34,200,156,12,"go out sendStr",12,0);
	myfree(SRAMIN,p);

}
void TcpSendWav(u8 *pname)
{
		FIL* fmp3;
		u16 br;
		int j;
...
char *reciveStr()
{
	int rlen;
	u8 *p;
	p = mymalloc(SRAMIN,30);//申请内存
	sprintf((char*)p,"go to recive\r\n");//测试数据
	...
	sprintf((char*)p,"go uot recive\r\n");//测试数据
	Show_Str(30+54,200,200,12,p,12,0);
	myfree(SRAMIN,p);
		return NULL;
}
...
void qiuqiuni(char *ssid, char *password, char *ip, char *port)
{
	int i,netpro = 1;
	int jisu = 0;
	char * temp;
	...
	u8 constate=0;	//连接状态
	POINT_COLOR=RED;
	Show_Str(40,55,200,16,"start  check",16,0); 
	while(atk_8266_send_cmd("AT","OK",20))//检查WIFI模块是否在线
	{
		atk_8266_quit_trans();//退出透传
		atk_8266_send_cmd("AT+CIPMODE=0","OK",200);  //关闭透传模式	
	...
	Show_Str(40,100,200,16,"everything is ok",16,0); 
	Show_Str_Mid(0,30,"ATK-ESP WIFI-STA 测试",16,240);
	...
	while(atk_8266_send_cmd(p,"WIFI GOT IP",300));					//连接目标路由器,并且获得IP
			...
			sprintf((char*)p,"AT+CIPSTART=\"TCP\",\"%s\",%s",ip,port);    //配置目标TCP服务器
			while(atk_8266_send_cmd(p,"OK",200))
			{
					LCD_Clear(WHITE);
					POINT_COLOR=RED;
			...
			Show_Str_Mid(0,80,"ATK-ESP 连接TCP Server succes",12,240); //连接失败	
LCD_Clear(WHITE);
			POINT_COLOR=RED;
			Show_Str_Mid(0,30,"ATK-ESP WIFI-STA 测试",16,240);
			Show_Str(30,50,200,16,"正在配置ATK-ESP模块,请稍等...",12,0);			
			...
			{	
				...
				{ 
					...
					Show_Str(30,130,180,190,USART3_RX_BUF,12,0);//显示接收到的数据  
					USART3_RX_STA=0;
					sendStr(USART3_RX_BUF);
					delay_ms(10);// avoid loop
					USART3_RX_STA=0;
