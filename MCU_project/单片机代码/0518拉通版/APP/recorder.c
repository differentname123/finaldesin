...
{	 
	...
	sprintf(p,"ÕıÔÚÂ¼ÖÆ£º0:/RECORDER/%s",(char*)USART3_RX_BUF);
	Show_Str(30,230,500,12,p,12,0); 			//ÏÔÊ¾½ÓÊÕµ½µÄÊı¾İ³¤¶
	myfree(SRAMIN,p);
...
int send_wav()
{
	...
	if (flag1 == -1)
		return 0;
	sprintf((char*)pname1,"0:RECORDER/%s",time);
	Show_Str(30+54,520,200,16,(u8*)pname1,16,0);
	Show_Str(30+250,520,156,16,(u8*)"ÕıÔÚÉÏ´«",16,0);
	//printf("TcpSendWav:%s", pname);
	TcpSendWav(pname1);
	//Show_Str(30+54,640,156,12,(u8*)"--go out sendwav",12,0);
	myfree(SRAMIN,pname1);
	//Show_Str(30+54,620,156,12,(u8*)"--pname1",12,0);
	
	myfree(SRAMIN,time);
	//Show_Str(30+54,660,156,12,(u8*)"--time",12,0);
	return 1;
 	   	...
 	   	while(rval==0)
		{
			...
			{		
				case WKUP_PRES:	//STOP&SAVE
					LCD_Fill(30+34,500,300,500+16,WHITE);
					while(flag!=0)
					{
						//xunhuan++;
						flag = send_wav();
						delay_ms(5000);// ±ÜÃâÇëÇóÌ«¿ìµ¼ÖÂ·şÎñ¶ËÃ»Íê³ÉÉÏÒ»¸öµÄ×ª»»¶ªÊ§´Ë´ÎÇëÇó

						if (flag == 0)
						{
							Show_Str(30+34,500,156,16,(u8*)"ÒÑÈ«²¿ÉÏ´«Íê±Ï",16,0);
						}
					}
					LCD_Fill(30+34,500,500,500+50,WHITE);
					break;	 
				case KEY0_PRES:	//REC/PAUSE
					LCD_Fill(30+34,500,300,500+16,WHITE);
					record_voice(3);
				
					LCD_Fill(30,250,900,400,WHITE);
					flag = 1;
					break; 
				case KEY1_PRES:	//AGC-
...
//¶ÁÈ¡Êı¾İ			  
			...
			{			
				...
				sprintf((char*)p,"µ±Ç°×´Ì¬:¿ÕÏĞ %d",timecnt);	
				Show_Str(30+34,500,156,16,p,16,0);
