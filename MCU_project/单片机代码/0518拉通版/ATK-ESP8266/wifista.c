...
PRESTA:
	...
	else     //TCP
	{
		if(netpro&0X01)     //TCP Client    透传模式测试
		{
			...
			sprintf((char*)p,"AT+CIPSTART=\"TCP\",\"%s\",%s","192.168.177.115","8086");    //配置目标TCP服务器
...
void connect_ip(char *ip,char *port)
{
	...
	atk_8266_send_cmd("AT+CIPMUX=0","OK",20);   //0：单连接，1：多连接
	Show_Str(40,250,200,16,"start connect ip",16,0); 
	sprintf((char*)p,"AT+CIPSTART=\"TCP\",\"%s\",%s",ip,port);   
	while(atk_8266_send_cmd(p,"OK",300))
	{
			LCD_Clear(WHITE);
			POINT_COLOR=RED;
			sprintf((char*)p,"try times: %d",times++); 
			Show_Str_Mid(0,40,p,16,240);
			Show_Str_Mid(0,80,"ATK-ESP 连接TCP Server失败",12,240); //连接失败	
	}

	Show_Str(40,300,200,16,"connect ip ok",16,0); 	
}
u8 zxh_atk_8266_wifista_test1(void)
{
	//u8 netpro=0;	//网络模式
	u8 key;
	u8 timex=0; 
	...
	p=mymalloc(SRAMIN,32);							//申请32字节内存
	atk_8266_send_cmd("AT+CWMODE=1","OK",50);		//设置WIFI STA模式
	Show_Str_Mid(0,30,"设置WIFI STA模式",16,240); 
	atk_8266_send_cmd("AT+RST","OK",20);		//DHCP服务器关闭(仅AP模式有效) 
	delay_ms(1000);         //延时3S等待重启成功
	delay_ms(1000);
	delay_ms(1000);
	...
	//设置连接到的WIFI网络名称/加密方式/密码,这几个参数需要根据您自己的路由器设置进行修改!! 
	sprintf((char*)p,"AT+CWJAP=\"%s\",\"%s\"",zxh_wifista_ssid,zxh_wifista_password);//设置无线参数:ssid,密码
	while(atk_8266_send_cmd(p,"WIFI GOT IP",300));					//连接目标路由器,并且获得IP
PRESTA:
	netpro|=1;	//选择网络模式
		if(netpro&0X01)     //TCP Client    透传模式测试
...
u8 zxh_atk_8266_wifista_test(void)
{
	//u8 netpro=0;	//网络模式
	u8 key;
	u8 timex=0; 
